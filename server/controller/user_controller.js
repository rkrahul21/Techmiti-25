...
module.exports.profile = async function (req, res) {
  try {
    ...
    const user = await User.findById(req.user._id, 'name email techmitiId teams phone isPaymentVerified -_id').populate({path: 'teams', select: 'team_name event_name -_id'});;
  ...
  try {
    ...
    ACL: 'public-read' 
      ...
      if (!existingEmailUser && !existingPhoneUser) {
        ...
        if (req.file) {
          user.receipt = data.Location;
          await user.save();
        }

        res.status(200).json({
          data: {techmitiId: user.techmitiId},
